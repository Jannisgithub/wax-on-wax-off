...
6. Safety and wording:
   We revised app copy to avoid misleading or exaggerated claims. The app now uses neutral wording such as estimated reclaimable space, review before removing, and move to Trash. Files are never removed without user confirmation, HIGH cleanup requires typing DELETE, Application Leftovers are never selected automatically, and system-managed or tool-managed areas are never automatic cleanup issues. Large backups, archives, simulator data, Docker data, and tool environments can appear only as manual storage insights.
