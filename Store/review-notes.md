...
Build 14 removes any path that could cause administrator authentication. The first-run folder picker now accepts only the signed-in user account Home folder and rejects /Users, system roots, lookalike folders, and other user folders. Cleanup analysis and apply also skip files or folders that are not owned by the signed-in user, including items moved through NSWorkspace.recycle, so the app never asks for administrator privileges.
